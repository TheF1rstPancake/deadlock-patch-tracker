...
Default overview sort is **A–Z** and shows **every hero** (or every item on the Items tab). **Recent volatility** (buff+nerf events in the **last 5 ingested patches**; hover the **?** — not a ~30-day window) and **Total touches** remain as chips; those ranked sorts may top-slice with **Show all**. The heatmap still windows to the **newest 10 patches**; **All patches** expands. Entity detail keeps the **full patch history** on a diverging **percentile** chart (buffs up, nerfs down; axis about −100…0…+100). Sign is direction; magnitude is percentile **within that direction’s peer set** (buffs among buffs, nerfs among nerfs; heroes and items never mixed). Two lenses, same visual language:
...
If fewer than 5 peers that side, the UI shows a rank fraction (`2 of 4`) instead of a fine percentile. **Counts** is an optional line-volume toggle (not “how hard”). A short **Hardest hits** list under the chart follows the **active** lens. Approximate extent weights stay in the drawer/hover; **buff/nerf line cards** lead with **absolute roster normalization** (`Roster P25 → P32 (ΔP +6)` on the live heroes+items property pack) so a +50% catch-up below median does not look like a spike. Relative % and same-aspect delta peer rank (Across / That day) sit under that. **Cumulative: approx extent** plots the older relative-% weights as a gold running signed line on a **separate right axis** (`Cumulative approx extent (%)`) so a loud 3/6 can jump while percentile bars stay even — labels make the two encodings obvious. **Cumulative: counts** is a running signed net of line counts (left axis = counts; same scale is fine). Axis titles + a one-line legend name both encodings; the gold line is never called a percentile. Structural/qualitative lines use a heavier stand-in than plain unmeasured lines; not win-rate. Click a buff/nerf bar (or the patch group) for the concrete lines. Fixes stay dots, not bar height. Heatmap **Absolute | Relative** is unchanged (Relative is still within-column, not a detail-chart mode).
...
| `data/roster-baselines.json` | Live heroes+items property bags for absolute extent (P_before → P_after). Regenerated by `npm run build-roster-baselines` from [deadlock-api](https://api.deadlock-api.com/v1/assets/heroes) heroes + items — check this file in so Pages never calls the API from the browser. |
